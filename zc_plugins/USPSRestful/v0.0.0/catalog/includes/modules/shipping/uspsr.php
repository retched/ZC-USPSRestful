...
class uspsr extends base
{
    ...
    public function install()
    {
        ...
        if (!defined('MODULE_SHIPPING_USPSR_INSTALL')) {
            ...
            global $db;
            $db->Execute("INSERT INTO admin_pages (page_key, language_key, main_page, page_params, menu_key, display_on_menu, sort_order) VALUES ('uspsUninstall', 'BOX_USPSR_UNINSTALLER', 'FILENAME_USPS_UNINSTALL', '', 'tools', 'Y', 600)");
        }
                ...
                if (defined('MODULE_SHIPPING_USPSR_TYPES')) {
                    ...
                    for ($i = 0; $i <= (count($config_methods) - 1); $i++) {
                        ...
                        if ($method == 22) { // On the 22nd method on the list, break and add data for the First-Class Mail International Letter
                            array_splice($config_methods, $i, 0, [0, ((defined('SHIPPING_WEIGHT_UNITS') && SHIPPING_WEIGHT_UNITS === 'kgs') ? 1.8143 : 4), "0.00"]);
